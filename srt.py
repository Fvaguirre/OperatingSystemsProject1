...
def fillReadyQueue(p_scheduler, global_time, l, alpha, context_switch_time):
	#print("\tIN FILLREADYQUEUE")
	#check if there are no more processes
	if not p_scheduler.processes.empty():
		#print("\tthere are more processes")
		while not p_scheduler.processes.empty() and p_scheduler.processes.queue[0][2].arrival_time <= global_time:
			#print("\tTHE GLOBAL TIME IS %d AND PROCESS %d ARRIVES AT %d" %(global_time, p_scheduler.processes.queue[0][2].pid, p_scheduler.processes.queue[0][2].arrival_time))
			...
			if (current_node[2].tau == 0):
				...
				#print("\t FIRST TAU = %f" %current_node[2].tau)
			else:
				...
				#print("\t NEW TAU = %f" %current_node[2].tau)
			...
			#global_time = runContextSwitch(p_scheduler, global_time, context_switch_time, alpha)
	return global_time
...
def incrementWaitTime(p_scheduler):
	if not p_scheduler.ready_queue.empty():
		for proc in p_scheduler.ready_queue.queue:
			...
			#print("    INCREMENTING WAIT TIME")


def runContextSwitch(p_scheduler, global_time, context_switch_time, alpha, l, time_change):
	#print("\tPerforming context switch at time %d" %global_time)
	...
	while cs_time < .5*context_switch_time:
		...
		checkBlockingQueue(p_scheduler, global_time, alpha, context_switch_time, l, time_change)
		global_time += 1
		#print("\t time: %dms" %global_time)
...
"""
def readyQueueToRunning(p_scheduler, alpha, global_time, context_switch_time, l, time_change):
	#print("\tIN READYQUEUETORUNNING")
	#print("     readyQueuetoRUnning global time: %d" %global_time)
	if_switched = False
	if not p_scheduler.ready_queue.empty():
		#print("\tthe ready queue is not empty")
		if p_scheduler.running is None:
			#there is no process currently running
			#print("\tthere is no process currently running")
			...
			r_q = p_scheduler.returnPrintableReadyQueue()
			global_time, if_switched = runContextSwitch(p_scheduler, global_time, context_switch_time, alpha, l, time_change)
			p_scheduler.running = current_node[2]
			...
			if (p_scheduler.running.remaining_time < p_scheduler.running.cpu_burst_times[p_scheduler.running.curr_cpu_burst]):
				...
				#print("  time finished : %d" %p_scheduler.running.time_finished)
			else:
			...
			#print("running process is: %d" %p_scheduler.running.pid)
			current_node = p_scheduler.ready_queue.get()
			#check the new node's remaining time against the currently running process remaining time
			#print("               current process remaining time: %d " %current_node[2].remaining_time)
			#print("                     running process remaining time: %d" %p_scheduler.running.remaining_time)
			if (current_node[2].remaining_time == -1):
				current_node[2].remaining_time = current_node[2].cpu_burst_times[current_node[2].curr_cpu_burst]
			#print("\tnew process remaining time: %d\n\tcurrent running process remaining time: %d" %(current_node[2].remaining_time, p_scheduler.running.remaining_time))
			#if (current_node[2].remaining_time < p_scheduler.running.remaining_time):
			...
			if(current_node[2].tau < new_priority):
				...
				#once for removing process in there
				global_time, if_switched = runContextSwitch(p_scheduler, global_time, context_switch_time, alpha, l, time_change)
				#print("          Global time 1: %d " %global_time)
				#once for putting new process in
				global_time, if_switched = runContextSwitch(p_scheduler, global_time, context_switch_time, alpha, l, time_change)
				#print("          Global time 2: %d " %global_time)
				#print("                GLOBAL TIME IN READY TO RUNNING: %d" %global_time)
...
def decrementRunning(p_scheduler, global_time, alpha, context_switch_time, l, time_change):
	#print("\t     IN DECREMENTRUNNING")
	#print("           decrementRunning global time %d" %global_time)
	if_switched = False
	if not p_scheduler.running is None:
		p_scheduler.running.remaining_time -= 1
		#print("                                          remaining time: %d" %p_scheduler.running.remaining_time)
...
def checkIO(proc, alpha, p_scheduler, global_time, context_switch_time, l, time_change):
	preempt = 0
	flagged = False
...
def checkBlockingQueue(p_scheduler, global_time, alpha, context_switch_time, l, time_change):
	preempt = 0
	if  len(p_scheduler.blocking) > 0:
			...
			for proc in p_scheduler.blocking:
				...
				preempt, global_time = checkIO(proc, alpha, p_scheduler, global_time, context_switch_time, l, time_change)
...
def runSRT(processes, num_processes, context_switch_time, alpha, l):
	...
	while jobs_completed < num_processes:
		...
		global_time, if_switched1 = readyQueueToRunning(p_scheduler, alpha, global_time, context_switch_time, l, time_change)
		#print("  if switched 1 = %s" %if_switched1)
		global_time, if_switched2 = decrementRunning(p_scheduler, global_time, alpha, context_switch_time, l, time_change)
		...
		if_switched3 = False
		preempt, global_time = checkBlockingQueue(p_scheduler, global_time, alpha, context_switch_time, l, time_change)
		if (preempt == 1):
			#global_time -= 1
			#gonna try a context switch here for removing the current running process
			run_proc = p_scheduler.running
			global_time, if_switched3 = runContextSwitch(p_scheduler, global_time, context_switch_time, alpha, l, time_change)
			#print("            global time after preempt context switch: %d" %global_time)
			#p_scheduler.running = ru
